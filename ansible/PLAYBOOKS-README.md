...
Sets `CONSUL_HTTP_ADDR`, `CONSUL_HTTP_TOKEN`, `NOMAD_ADDR`, and `NOMAD_TOKEN` by reading the first server IP from `inventory.ini` and the token values from the bootstrap token files in the `ansible/` directory.
...
Setting cluster environment variables (server: 1.2.3.4):
  ...
  CONSUL_HTTP_TOKEN=(set from consul-bootstrap-secret-id.txt)
  NOMAD_ADDR=http://1.2.3.4:4646
  NOMAD_TOKEN=(set from nomad-bootstrap-secret-id.txt)
```
...
4. `consul_acl_bootstrap.yaml` — bootstraps Consul ACL; writes `consul-bootstrap-secret-id.txt` to `ansible/`
5. `dnsmasq.yaml` — configures `.consul` DNS forwarding on all nodes
6. `nomad_servers.yaml` — Nomad server agents on `[servers]`
7. `nomad_clients.yaml` — Nomad client agents on `[clients]`
8. `nomad_acl_bootstrap.yaml` — bootstraps Nomad ACL; writes `nomad-bootstrap-secret-id.txt` to `ansible/`
9. `consul_nomad_integration.yaml` — Consul ACL resources for Nomad service discovery and workload identity; reconfigures Nomad agents with the `consul {}` block
10. Post-task summary — prints SSH/UI access details and all cluster token SecretIDs
...
**Prerequisites:**
- `consul-bootstrap-secret-id.txt` present in `ansible/`
...
**Prerequisites:**
- `consul_nomad_service_discovery.yaml` completed
- `consul-bootstrap-secret-id.txt` and `nomad-consul-server-secret-id.txt` present in `ansible/`
...
> **Warning:** This is destructive and irreversible. Nomad, Consul, Docker, CNI, and dnsmasq will be removed from all cluster nodes. All bootstrap token files will be deleted from the control machine.
...
| 7 | `teardown_tokens` | `localhost` | All bootstrap token files from the `ansible/` directory |
...
|------|----------|---------------|-------------|
| `ansible/roles/nomad/defaults/main.yaml` | `nomad_binary_version` | `2.0.3` | Nomad binary version to download and install |
...
| Variable | Default in playbook | Description |
|----------|--------------------| ------------|
| `nomad_binary_version` | `2.0.3` (role default) | Nomad version; set in `roles/nomad/defaults/main.yaml` |
